...
docker build -f docker/slither/Dockerfile -t smart-contract-audit-slither:local .
```
...
docker run --rm -p 3000:3000 \
  ...
  -e AUDIT_SLITHER_DOCKER_IMAGE=smart-contract-audit-slither:local \
...
- `/mcp` 需要 `Authorization: Bearer <token>`
- `/api/*`、`/` 和 `/healthz` 可直接访问
- Web Console 左侧的 Token 输入框主要用于手动调用 `/mcp` 时保存 Bearer Token
